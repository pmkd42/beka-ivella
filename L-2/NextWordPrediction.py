...
def trainModel():
    ...
    for line in open("/home/pammi/Desktop/test.txt"):
...
def next_word(tpl):
    ...
    if(type(tpl) == tuple): #incoming words are combination of two words.. find next word now based on transitinos(?)
...
if __name__ == '__main__':
    ...
    import getch
    trainModel()
    print("Enter for hit-based search, Tab for stochiastic search:")
    l_out = "ibhumibannada"       #press return for disbander, tab for direct L-2
    tokens = ["i", "bhumi", "bannada", "bumibannada", "ibhum"]
    hashmap = [["i",float(2/85)], ["bhumi", float(32/85)], ["bannada", float(10/85)], ["ibhum", 0], ["bumibannada", 0]]
    wordlist = []
    hit = input()
    for ele in hashmap:
        if(ele[-1] > 0.1):
    ...
    #print(wordlist)
    print("Direct hit found:[",wordlist[:2], "]")
    print("Similarity:" ,float(hit == hit))
    c=' '
    sent = ''
    last_suggestion = []
    while (c != '\r'):  # stop when user preses enter
        if (c!='\t'):     #if previous character was tab, then it write 1st suggestion no wait for inputing
            c=getch.getch()  #inputing charcter by character
        else:
            c=' '
        if c!='\t':
        ...
        if c==' ':
            ...
            if len(tkns)<2:  #if 2nd word want to write after 1st
                ...
                last_suggestion=next_word((tkns[-2],tkns[-1]))  #last back to first (tkns[-1], tnks[-2])
                print(last_suggestion,end=' ',flush=True)
